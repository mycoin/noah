 ...
 * </pre>
 *
 * This bean must be included in the application context of any application using Spring's {@link VelocityView} for web
 * MVC. It exists purely to configure Velocity; it is not meant to be referenced by application components (just
 * internally by VelocityView). This class implements {@link VelocityConfig} in order to be found by VelocityView
 * without depending on the bean name of this configurer. Each DispatcherServlet may define its own VelocityConfigurer
 ...
 * {@code spring.jar}), which makes all of Spring's default Velocity macros available to the views. This allows for
...
public class VelocityConfigurer extends VelocityEngineFactory
	...
	private static final String SPRING_MACRO_LIBRARY = "com/breakidea/noah/web/velocity/spring.vm";
	...
	/**
	 * Set a pre-configured VelocityEngine to use for the Velocity web configuration: e.g. a shared one for web and
	 * email usage, set up via {@link com.breakidea.noah.web.velocity.VelocityEngineFactoryBean}.
	 * <p>
	 * Note that the Spring macros will <i>not</i> be enabled automatically in case of an external VelocityEngine passed
	 * in here. Make sure to include {@code spring.vm} in your template loader path in such a scenario (if there is an
	 * actual need to use those macros).
	 * <p>
