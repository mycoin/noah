 ...
 *
 * <p>
 * The {@code url} property should be set to the content template for the view,
 * and the layout template location should be specified as {@code layoutUrl}
 * property. A view can override the configured layout template location by
 * setting the appropriate key (the default is "layout") in the content
 * template.
 *
 * <p>
 * When the view is rendered, the VelocityContext is first merged with the
 * content template (specified by the {@code url} property) and then merged with
 * the layout template to produce the final output.
 *
 * <p>
 * The layout template can include the screen content through a VelocityContext
 * variable (the default is "screenContent"). At runtime, this variable will
...
public class VelocityLayoutView extends VelocityView {
     ...
     */
    public static final String DEFAULT_LAYOUT_URL = "blankLayout.vm";
     ...
     * {@code #set($layout = "blankLayout.vm" )}
     * <p>
     * Default key is {@link #DEFAULT_LAYOUT_KEY "screenLayout"}, as illustrated
     * above.
     * 
     * @param layoutKey
     ...
     * <p>
     * Default is {@link #DEFAULT_SCREEN_CONTENT_KEY "screenContent"}: accessed
     * in VTL as {@code $screenContent}.
     * 
